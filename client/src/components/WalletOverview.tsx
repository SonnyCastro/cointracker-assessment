...
const WalletOverview = (): React.JSX.Element => {
  const {
    selectedWallet,
    wallets,
    setEditMode,
  ...
  const balanceData = useMemo((): BalanceData => {
    ...
    const netPosition = transactions.reduce(
      (total, tx) => total + tx.balance,
      0
    )
  ...
  const formatTransactionAmount = useCallback((balance: number): string => {
  ...
  const handleSync = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // Sync wallet - this now updates wallets internally, no refetch needed
    ...
    return (
      <div className='wallet-overview'>
        <div className='no-wallet-selected'>
          ...
          <p>
            Select a wallet from the sidebar to view details and transactions.
          </p>
        </div>
      </div>
    )
    ...
    <div className='wallet-overview'>
      ...
      <div className='wallet-header'>
        ...
        <div className='wallet-actions'>
          <button
            className={`btn btn-primary ${isSyncing ? "syncing" : ""}`}
            onClick={handleSync}
            disabled={isSyncing}
          >
        ...
        {balanceData.isNormalized && (
          <span
            ...
            title={`Original balance: ${
              balanceData.originalBalance?.toFixed(2) || "0.00"
            } BTC`}
          >
            *
          </span>
            ...
            <div className='error-content'>
              ...
              {transactionsIsRetrying && (
                <div className='retry-info'>
                  Retrying in{" "}
                  {Math.ceil(
                    Math.min(1000 * Math.pow(2, transactionsRetryCount), 5000) /
                      1000
                  )}{" "}
                  seconds... (Attempt {transactionsRetryCount + 1}/5)
                </div>
              )}

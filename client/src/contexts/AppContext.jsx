...
const appReducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTIONS.REMOVE_SYNCING_WALLET:
      return {
        ...state,
        syncingWallets: new Set([...state.syncingWallets].filter(id => id !== action.payload))
      }
...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!walletsLoading && wallets && wallets.length > 0 && !state.selectedWallet) {
      dispatch({ type: ACTIONS.SET_SELECTED_WALLET, payload: wallets[0].id })
    }
  }, [wallets, walletsLoading, state.selectedWallet])
  ...
  const actions = {
    setSelectedWallet: useCallback((walletId) => {
    ...
    setEditMode: useCallback((isEditMode) => {
    ...
    setIsSyncing: useCallback((isSyncing) => {
      dispatch({ type: ACTIONS.SET_IS_SYNCING, payload: isSyncing })
    }, []),

    addSyncingWallet: useCallback((walletId) => {
      dispatch({ type: ACTIONS.ADD_SYNCING_WALLET, payload: walletId })
    }, []),

    removeSyncingWallet: useCallback((walletId) => {
      dispatch({ type: ACTIONS.REMOVE_SYNCING_WALLET, payload: walletId })
    }, []),

    setPendingWalletSelection: useCallback((walletId) => {
      dispatch({ type: ACTIONS.SET_PENDING_WALLET_SELECTION, payload: walletId })
    }, []),

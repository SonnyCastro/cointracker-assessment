...
const { wallets, selectedWallet, setSelectedWallet, isEditMode, setEditMode } = useAppContext()
```
...
const balanceData = useMemo(() => { /* calculations */ }, [transactions])
const handleSync = useCallback(async () => { /* logic */ }, [dependencies])
```
...
export const AppProvider = ({ children }) => {
  ...
  const { wallets, loading, error, createWallet, deleteWallet, syncWallet } = useWallets()
  const { transactions, loading: txLoading, error: txError } = useWalletTransactions(selectedWallet)
  ...
  return (
    <AppContext.Provider value={{ ...state, ...actions, wallets, transactions }}>
      {children}
    </AppContext.Provider>
  )
}
...
export const useRetry = () => {
  ...
  const retryWithBackoff = useCallback(async (fn) => {
